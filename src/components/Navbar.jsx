...
function Navbar() {
    ...
    const links=['/', '/dashboard', '/employee', '/roles-permission', '/time-attendance/', '/geo-fence', '/leave', '/reports'];
        ...
        <nav className="fixed w-24 h-screen bg-blue-900 text-slate-200">
            <ul className="flex flex-col list-none text-4xl items-center leading-[-2rem] pt-4">
                {icons.map((item, index) => (
                    <li key={index} className='flex flex-col items-center border-b w-3/4'>
                        <NavLink to={links[index]} className={({ isActive }) => `${isActive ? "text-white" : "text-slate-400"} flex flex-col items-center hover:text-slate-300 transition py-3`}>{item}
