...
function InputField(props) {
    const { name, type, imp, accept } = props.params
    return (
        <>
            <div className="rounded-md border-2 relative border-blue-500 w-full flex py-2">
                ...
                {type === 'file' &&
                    <>
                        <input className='rounded-md w-full h-full p-2 outline-none bg-transparent font-thin opacity-0' type={type} name={name} id={name} accept={`application/${accept && accept}`}
                            onChange={(e) => {
