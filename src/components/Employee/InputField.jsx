...
function InputField({ params, formData }) {
    ...
    return (
        <div className="rounded-md border-2 relative border-blue-500 w-full flex py-2 text-base ">
            <label htmlFor={name} className="leading-none absolute top-[-3%] -translate-x-1/2 left-1/2 -translate-y-1/2 px-3 text-xs bg-zinc-700 capitalize">
                {name.replace(/_/g, ' ')} {imp && <span className='text-red-600 text-sm leading-none'>*</span>}
            </label>
            ...
            {type === 'file' &&
                <>
                    <input
                        ...
                        disabled={readOnly}
                    />
                    <p htmlFor="" className=' absolute top-1/2 -translate-y-1/2 px-2 whitespace-nowrap overflow-hidden text-ellipsis w-full text'>
